...
function TaskScheduler:queueTasks(planner, recipe, crafts, summary, parent, dest, destSlot, depth, visiting, skipDependencies)
    local batches = TaskAssigner.buildBatches(planner, recipe, crafts)
    local remaining = crafts
    for i = 1, #batches do
        local batch = batches[i]
        local task = TaskAssigner.createTask(planner, recipe, batch, summary, parent, dest, destSlot, depth)
        self:registerTask(summary, task)
        if not skipDependencies then
            planner:attachDependencies(task, recipe, depth, visiting, batch, summary)
...
function TaskScheduler:logSummary(summary)
    ...
    local machinePool = self.server and self.server.machinePool or nil
    ...
    for machineType, entry in pairs(summary.machineStats) do
        local count = machinePool and machinePool:countMachines(machineType) or 0
        if count > 0 then
    ...
    local idle = totalTime - (totalRun / math.max(1, (criticalMachine and machinePool:countMachines(criticalMachine) or 1)))
    if idle < 0 then
        idle = 0
    end
    ...
    if criticalMachine and machinePool then
        local count = machinePool:countMachines(criticalMachine)
        if count > 0 then
